...
from peertube import Client, AuthenticatedClient
from peertube.api.video import get_categories, get_languages, get_licences
...
def basic_client_usage():
    ...
    print(f"Client created with base URL: {client._base_url}")
    print("This client can be used for public endpoints that don't require authentication.")
...
def authenticated_client_usage():
    ...
    print(f"Authenticated client created with base URL: {auth_client._base_url}")
...
def advanced_client_configuration():
    ...
    print(f"  - Timeout: {client._timeout} seconds")
    print(f"  - SSL verification: {client._verify_ssl}")
    print(f"  - Follow redirects: {client._follow_redirects}")
    print(f"  - Custom headers: {client._headers}")
    print(f"  - Cookies: {client._cookies}")
    print()
...
def main():
    ...
    api_modules = [
        "accounts", "video", "videos", "users", "session", "config",
        "search", "video_channels", "video_playlists", "my_user",
        "my_subscriptions", "my_notifications", "register"
    ]
